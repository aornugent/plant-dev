# P1 minimal (fast first read): does a field-aware cohort SHAPE beat uniform at
# matched count on intense_storms? Runs are ~110s each, so keep it small.
...
Jf <- function(sched=NULL){ scm <- SCM("TF24","TF24_Env")(mkp(sched), mkenv(), PLAIN); scm$run(); sum(scm$offspring_production) }
d0 <- SCM("TF24","TF24_Env")(mkp(), mkenv(), PLAIN); d0$run(); baseN <- d0$patch$species[[1]]$node_times; N <- length(baseN)
cat(sprintf("N=%d\n", N)); flush(stdout())
Jref <- Jf(seq(0,tmax,length.out=N*3)); cat(sprintf("Jref(3x uniform, M=%d)=%.5e\n", N*3, Jref)); flush(stdout())
...
for (fn in names(fams)) { e <- abs(Jf(fams[[fn]])-Jref)/abs(Jref); cat(sprintf("  %-10s relerr=%.4f\n", fn, e)); flush(stdout()) }
cat("ALLDONE\n")
