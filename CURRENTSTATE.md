...
> **This register was written against the v3 line and is being carried onto v4.** Every entry
> below was established against the commits in the *measured-against* column. The v4 branch is
> based on the package trunks instead, and the leaf has moved out of `plant` into `phylloptim`.
> **An entry is not a claim about the v4 base until it has been re-established there.** See
> [`V4-INTEGRATION.md`](V4-INTEGRATION.md) for which entries survive the move and which have to
> be re-measured.

| | measured against (v3) | this branch pins (v4) |
|---|---|---|
| superproject | `459e262` on `claude/odelia-ad-tape-reverse-496fuf` | `ad/v4-integration` |
| `plant` | `600e3ebd` (`v1.2.1-427`) on `p3/wave5` | `39c0c657` on `develop` |
| `odelia` | `a3bcf58` on `p3/odelia-integration` | `880a1c2f` on `master` (0.3.0) |
| `phylloptim` | — (leaf lived inside `plant`) | `3345795` on `master` |
| `regnans` | — | `e53713a9` on `master` |
| `logpile` | `cdef780f` on `main` | `cdef780f` on `main` |

The v3 plant pointer was `66029469`, described as `600e3ebd` plus a census abscissa fix. **That
commit was never pushed and no longer exists** in any repository or clone. The fix it carried is
lost; see departure 13 below, which reverts to two open paths.
...
| 13 | Two census quadratures take the wrong abscissa. **Both halves are open again** — the C++ fix was lost with `66029469` | §9 | read; error measured at about 4 percent — a defect in the objective |
...
it is two paths, on opposite sides of the R boundary — both of which are open on this base.**

> **`Species::census` is NOT fixed on the v4 base.** The C++ fix existed only in plant commit
> `66029469`, which was never pushed and is unrecoverable. What follows is the recorded result of
> that fix, kept because it is the specification for redoing it and the gate that checked it still
> exists.
>
> The fix integrated over `quadrature_abscissa()` rather than `height()`, which corrects the measure
> on the birth-date coordinate and removes the ordering hazard there at the same time, since birth
> dates cannot invert. It was gated two ways against a build of the parent commit: the **height**
> coordinate was unchanged to every printed digit — leaf area 4.1526002307, above-ground mass
> 12.3501592205, stem area 0.0041602088 — and the **birth-date** coordinate moved, leaf area
> 12.8932796075 to 3.5059067337. Offspring was unchanged in both arms, so the reproduction integral
> is undisturbed. Configuration `scripts/measure/census-abscissa-gate.R`, lifetime 20,
> `lma = 0.0825`, `hmat = 5`, birth rate 20. `test-scm.R` passed 140 of 140.
>
> **Redoing it is the first code task on this branch.** The gate script is ported here, and the two
> arms above are the acceptance numbers — but they were measured against `600e3ebd`, not against the
> v4 base, so re-baseline the height arm before trusting the birth-date arm.
>
