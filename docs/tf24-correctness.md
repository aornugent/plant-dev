...
| `max(light, 1e-4)` | **disputed, and the dispute matters.** Report 07 §1.8 censuses 4.638% of 41 460 light-spline knot values at or below the floor, minimum exactly 0. But `L = exp(-A)` with `A` the leaf area *above* `z`, so `L` is minimised at the ground by construction — and measured directly from `Patch::compute_competition` over 141 introduction steps it runs **0.1657 to 1.0**, never within four orders of the floor (`../scripts/boundary_node.R`). Reaching `1e-4` needs about five times this stand's optical depth. So either §1.8 measures the **interpolant** rather than the profile — in which case the fitted cubic undershoots from 0.166 to below zero, far worse than report 03 §2's mild-undershoot account — or its provenance needs re-examining. **Until that is settled, this row justifies nothing**: it is either a live severance on 4.6% of the field or a spline defect with no physical severance at all | report 07 §1.8 against `scripts/boundary_node.R` |
| `max(0.0, spline(height))` undershoot guard | firing — the field's minimum is **exactly 0**, while the underlying profile's minimum is **0.1657**. So the guard is not trimming a small overshoot between knots; it is catching an interpolant that has left the function by more than the function's own range. Same dispute as the row above, and the same resolution settles both | report 07 §1.8 against `scripts/boundary_node.R` |
