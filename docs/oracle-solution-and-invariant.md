...
For feedback from both prior respondents. Domain-agnostic; no application knowledge is
assumed. The concrete system is one interpolator primitive plus two coupling fields
built on it, inside a method-of-characteristics solver — the invariant we seek is a
structural property of *any* such coupling, so nothing below depends on what the
...
characteristics that couple **only** through shared low-rank fields. Each field `S` is
a `k`-knot reconstruction (`k ≈ 17`, knot positions fixed) of an aggregate the
ensemble itself generates:

```
a(z) = Σⱼ wⱼ κ(z, xⱼ, stateⱼ; θ)     aggregate (a quadrature over the ensemble)
s(z) = ψ(a(z); θ)                     pointwise map (here ψ = exp)
S    = reconstruct{ s(z_m) }          k-knot interpolant, rebuilt each step
```

Each characteristic reads `S` **at its own coordinate** `xᵢ` — both the **value**
`S(xᵢ)` (enters its rates) and the **slope** `∂S/∂x(xᵢ)` (the transport/compression
term). So every characteristic is simultaneously a **source** and a **reader** of the
field, at a moving coordinate. There will be a second such field with the same
structure, one of which additionally carries its own ODE state (memory). The forward
value of the transport term is a stability-mandated finite difference; only its
derivative is in question.
...
field; a **null-channel** parameter with true sensitivity ≈ 0 exposes any spurious
...
treatment against a converged FD on a small ensemble (`N = 2`). Representative truth
values: a trajectory-moving parameter `D ≈ 319`; the coupling null-channel parameter
`C₀ ≈ 0.144`.
...
| **cavity, read-point** (analytic self-subtraction `S·e^{a_self}`) | ✗ 1.5× | ✓ **5–7%** |
| **cavity, deposit-level** (downdate self AT KNOTS, re-reconstruct) | ✓ **~1%** | ✗ **C₀ 94% low** |
...
1. **The error is a faithfulness bug, not a numerical one.** Forward values are
   ...
   an O(1) factor that does not shrink with FD step. That is categorically "the tape
...
2. **Self-reference is the load-bearing structure.** The spurious signal for a
   ...
   imprint* on the coarse field as it moves — a self-force. Leave-one-out fixes it, but
   only when done at the **deposit level** (downdate the source's own contribution at
   the knots and re-reconstruct); the analytic read-point cavity is quantitatively
...
3. **The two cavity variants fix opposite classes — and that is the crux.**
   Deposit-level LOO makes the direct params machine-close (`D` ~1%) but drives the
   coupling null-channel `C₀` to ~0 (94% low), because a clean self-exclusion also
   removes the characteristic's **genuine** self-coupling — the `O(k/N)` self-term,
   which at `N = 2` is most of `C₀`'s signal. Keeping self (no cavity) matches finite-N
   FD on `C₀` but leaves the direct params 2.3× hot.
...
- **"faithful discrete adjoint of the model-as-run"**: the finite-N model *does* let a
  characteristic feel its own field-gradient, so FD (`C₀ ≈ 0.144`) is truth and the
  self-term must be **kept**; the direct-param overshoot must then be cured *without*
  removing self (a stencil whose exact adjoint is well-conditioned, plus a query read
  that carries its motion).
- **"cavity / mean-field"**: the self-force is a coarse-grid artifact (Detweiler–
  Whiting regularization, particle-in-cell self-force, propagation-of-chaos), the
  correct gradient is the self-excluded one (`C₀ ≈ 0.008`), and finite-N FD is *not*
  the gold standard for coupling-only parameters.
...
that, once imposed, makes every coupled field correct by construction. Our candidate,
stated for critique:
...
>    block), "leave-one-out" is one structural downdate `c^{−i} = c − depositᵢ`, applied
>    the same way to the value read and the slope read — never per-channel, never
>    analytic-at-the-read-point.
...
>    is differentiable, rather than by a non-differentiable upwind/limiter stencil whose
>    exact adjoint staircases.
...
2. **The self-block decision (§2.3 / finite-N).** Is the `O(k/N)` self-coupling that a
   characteristic exerts on its own field-gradient **genuine structure of the
   model-as-run that a faithful adjoint must keep** (so finite-N FD is truth and the
   direct-param overshoot must be cured another way), or a **coarse-grid self-force to
   be excluded** (so the deposit-level cavity is right and finite-N FD is not the
   reference for coupling-only parameters)? Both frameworks were internally consistent;
...
3. **If keep-self is correct**, what is the well-conditioned, faithful realization of
   ...
   rather than land on FD? Is that overshoot a second faithfulness bug (the secant value
   and the analytic query-motion belonging to different reconstructions), which the
   single-reconstruction invariant would remove?

4. **Does the invariant compose to the second field for free**, including the case where
   that field carries its own ODE state (memory), so its self-block downdate must act
   over history — and is the affine/superposition self-downdate the only closed form, or
