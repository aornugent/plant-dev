# Does one event segment re-run from a stored state reproduce the forward pass?
# Run from /home/user/plant-dev:
...
for (m in c("K93", "FF16")) {
  ...
  print(data.frame(segment = r$probed, width_ok = r$width_ok,
                   max_reld = signif(r$max_reld, 3), max_abs = signif(r$max_abs, 3)))
}
