...
| 5 | [`oracle-consultation-inner-argmax-adjoint.md`](./oracle-consultation-inner-argmax-adjoint.md) | standalone (session 13): **broad characterisation** of reverse-mode gradients of the coupled multiscale IVP whose RHS contains a per-unit inner argmax solved to a fixed tolerance. Presents the whole system + all three trifecta symptoms of the *same* object (forward noise-floor step-collapse; `J` 2.4× non-monotone in τ; O(1) reverse-mode gap), both geometries (interior optimum *and* branch-fold corner), all three tape representations of `p*`, every refutation, and the well-conditioned resolvent. Deliberately does **not** lead with a candidate fix — asks for the structural read on accuracy×stability×correctness together, and what correctness reference is even right for a computation with a finite inner solve. Cross-informed by the multirate branches (`n5audm`/`r6dpwn`). CLEAN. | drafted, not sent |
