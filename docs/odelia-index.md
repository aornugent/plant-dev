...
|---|---|---|
| **the scan** | builds the coupling field `A` from the model's separable factors over the cohorts — exact, non-adaptive; its reverse is the one hand-written transpose (dot-product self-checked) | scan-coupling (#1) |
