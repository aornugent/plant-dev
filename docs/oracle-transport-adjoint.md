...
forward value is correct. Each is stated abstractly below: what it is, why naive
AD fails, and the technique (established or found) that fixes it.

Component 1 is written up in full, including the resolution reached with outside
input; it is the template for the analysis the others need. Components 2–6 are
stated more briefly. Each is self-contained and can be read on its own.
...
regularized limiter (`g_xθ ~ 1/ε`) this is huge (orders of magnitude above the
true gradient). The number is **exact, not rounding** — the numerator is
genuinely `O(1)` there. Recorded pointwise, `M(θ)` carries a sub-grid staircase
(steep ramps of slope `~1/h` separated by flats, because `h ≪` the point spacing
`Δx`); the exact discrete adjoint faithfully reports the local staircase slope,
which is not the macroscopic sensitivity wanted.

### What was tried
- **Differentiate the stencil on-tape** (exact discrete adjoint): consistent but
  ill-conditioned as above.
- **Use the exact analytic `∂g/∂x` throughout**: believed to be forward-unstable,
  but a control experiment refuted that — running the term through the primal
  path with a vanishing step (one-sided *and* centred) is stable and matches
  production. The apparent instability came from an implementation that **froze
  the coupling field** `S`, dropping the `∂g/∂S·dS/dx` term (a different,
  incomplete derivative). *There is no stability barrier.*
- **Keep the stencil value, inject the exact analytic derivative at fixed `S`**
  ...
  the coupling term → a consistent underestimate that grows with coupling
  strength.
...
2. **Inject the secant's value but FREEZE its θ-sensitivity.** Taping
   ...
   on the coupling channel — it overshoots the gradient severalfold. Its true
   contribution is negligible, so injecting the value and differentiating the
   *frozen* surrogate is both well-conditioned and accurate.

This is the "record the stabilization's value, differentiate a tamer surrogate"
(frozen-limiter) move, applied to the coupling secant. Result: the gradient
closed to sub-percent accuracy for the parameters that enter the velocity
directly, forward solution bit-identical.

### Residual / open
The freeze is exact only while `d(dS/dx)/dθ`'s true contribution is small. If a
functional makes it matter, it needs a *consistent* well-conditioned channel —
the standing candidate is taking the coupling difference over the actual point
spacing `Δx` (a true grid stencil, adjoint scale `O(1/Δx)`) instead of the
sub-grid probe `h`. Established art in the same family: differentiable / frozen
limiters and dual-consistent (SBP-SAT, entropy-stable) schemes; adjoints at
shocks (Giles & Ulbrich — exact discrete adjoints of shock-capturing schemes
...
as an off-tape value with these analytic partials attached, so the reverse pass
distributes adjoints to `θ` through the IFT formula. `∂F/∂y` and `∂F/∂θ` come
from AD of `F` itself (cheap, `F` is differentiable) — the only thing excluded
from the tape is the *search*.
...
### Abstract
A value is the optimum `v* = min_ψ (or max) φ(ψ, θ)`, or the optimizer `ψ*(θ)`
itself, produced by an inner optimization the outer AD cannot see through.

### Why naive AD fails
As in component 2, plus: unrolling the optimizer is memory-heavy and its
...
partials (same seam as component 2). One subtlety: if one of the differentiation
targets is itself a variable the objective is *also* optimized against
implicitly, its envelope partial `∂φ/∂(that variable)` is a genuine extra term
and must be included — it is easy to miss because the `ψ`-gradient being zero at
`ψ*` can be mistaken for "no sensitivity".
...
derivative of a **first-order approximation**. Differentiating a finite-difference
stencil `(f(x)−f(x−h))/h` on-tape amplifies by `1/h` anything non-smooth it
straddles; differentiating an under-resolved interpolant's slope compounds its
error.
...
**inject it with analytic partials** (components 2–3) when it is opaque. The audit
rule that catches the class: tag every site with "is this value a derivative?" —
a first-order "is the active scalar present?" check cannot see it.
...
### Why naive AD fails
AD returns *a* subgradient at the kink, correct almost everywhere but (a)
arbitrary exactly at the kink, and (b) actively dangerous when a kink sits on a
value that is itself differentiated or finite-differenced: an inner stencil
straddling the kink is amplified (components 1, 4), and the second derivative is a
spike.
...
(e.g. `½(x + √(x² + ε²))` for `max(0,x)`; smooth-min; logistic step). Choose the
sharpness so the value change is negligible but the second derivative stays
bounded — there is a real tension (sharper = smaller value change but steeper
`f''`). A kink *not* on a differentiated path (a pure guard, selector, or
finiteness test) needs no smoothing, but it must be written so it **reads the
value and never throws on an active intermediate** — fixed once at the operation's
definition, not per call site.
...
### Abstract
A field is reconstructed by interpolation/regression through sample points (e.g. a
spline), then queried at a point that is itself an evolving state variable of the
ODE. Two derivative channels exist: w.r.t. the **fitted data** (the sample
values), and w.r.t. the **query location** (the interpolant's analytic tangent).
...
**compounds across the time integration** (growing without bound as the horizon
lengthens). The forward *value* is unaffected — this is purely a
...
derivative** (evaluate at the stripped query value, carry only the data-channel
derivative). It is value-identical to a plain read, so it is a no-op for the
forward pass and cannot silently attach the dangerous tangent. Expose the
query-tangent read as an explicit, opt-in path for the rare caller that genuinely
wants it (a well-resolved reconstruction queried at a real differentiation input).
Where a query-*direction* derivative is genuinely needed on a rate path, get it
from a **secant** over the sample spacing, not the analytic tangent (this is the
`dS/dx` rule in component 1).
...
- **A correct forward value does not imply a correct derivative.** Several of
  these (1, 4, 6) are value-identical to the plain run and wrong only on the tape.
...
- **Classify by relationship to the derivative, not by site.** Off-the-derivative
  ...
  definition, so no call site special-cases it.
- **"Record the value, differentiate a tamer surrogate"** recurs (1, and the
  frozen-guard form of 5): when the exact derivative of a numerically-stabilized
  quantity is ill-conditioned, freezing it and differentiating a smooth stand-in
  is often both well-conditioned and accurate — but it is an approximation whose
  dropped term must be argued small, not assumed zero.
