...
| **live, and newly so** | the leaf's dark respiration at 25 °C, its photosynthetic capacity and its electron-transport maximum | it is the term net assimilation subtracts, and it arrived as a trait of its own when the leaf stopped deriving it from photosynthetic capacity. The leaf's constructor does not take it, so for a while it sat at the leaf's default with no parameter able to move it and no row. It is now carried at that same default and driven with the other leaf traits: measured on two fixtures, −0.033 to −0.305 on a leaf-area census against `beta2`'s 0.073 to 0.546, with every other column bit-identical. The two capacities were held out for a different reason — the leaf's temperature block cached their derived values under a key of the drivers alone, so a moved trait was reused rather than recomputed — and that key now covers every input of the block. They carry rows of 3.0e-03 to 1.2e-02 on the same census, again with every prior column bit-identical. **The most-measured pair of leaf physiological traits in the field read as absent until this was checked** |
