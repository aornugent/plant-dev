...
| **live, and newly so** | the leaf's dark respiration at 25 °C | it is the term net assimilation subtracts, and it arrived as a trait of its own when the leaf stopped deriving it from photosynthetic capacity. The leaf's constructor does not take it, so for a while it sat at the leaf's default with no parameter able to move it and no row. It is now carried at that same default and driven with the other leaf traits: measured on two fixtures, −0.033 to −0.305 on a leaf-area census against `beta2`'s 0.073 to 0.546, with every other column bit-identical |
| **absent — no row anywhere, ever** | every soil parameter — saturated conductivity, the retention curve's scale and exponent, the saturation, residual and ceiling constants, the infiltration pair, **times the layer count** if per-layer; the root depth shape; the two root-allocation constants; the crown shape; the leaf's photosynthetic capacity and its electron-transport maximum | **"What if the soil were sandier" cannot be asked.** Nor can the vertical structure of the root coupling. The crown shape is excluded for a numerical reason rather than a structural one — its derivative is `0^η · log 0` at the ground knot. **The two photosynthetic capacities are the newest and the least defensible.** Each is carried and passed to the leaf, and each is withheld from the differentiable set because the leaf's temperature block once cached its derived value under a key of the drivers alone, so a moved trait was reused rather than recomputed. That key now covers every input of the block, both of these among them, and a moved value is recomputed — measured, each moves profit (3.5e-02 and 3.9e-03 per unit at an interior point). So the obstacle is gone and the absence is now a row nobody has driven, which for the most-measured pair of leaf physiological traits in the field is the largest gap in this table |
