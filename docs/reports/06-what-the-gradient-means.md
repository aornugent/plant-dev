...
| **absent — no row anywhere, ever** | every soil parameter — saturated conductivity, the retention curve's scale and exponent, the saturation, residual and ceiling constants, the infiltration pair, **times the layer count** if per-layer; the root depth shape; the two root-allocation constants; the crown shape; the leaf's dark respiration at 25 °C | **"What if the soil were sandier" cannot be asked.** Nor can the vertical structure of the root coupling. The crown shape is excluded for a numerical reason rather than a structural one — its derivative is `0^η · log 0` at the ground knot. Dark respiration is the newest of these and arrived by a change in the leaf: it was the photosynthetic capacity times a fixed ratio, so it moved with a parameter that has a row, and it is now a trait of its own with its own temperature response and no strategy parameter setting it. It is the term subtracted inside net assimilation, not the tissue respiration the mass budget pays, so no other row reaches it |
