...
| **[v]** | **What the new tests add to every `R CMD check` leg, on three operating systems.** `tests/testthat.R` is `test_check("plant")`, so all of it runs in the check — the ladder runner is a developer convenience, not a gate. Measured end to end: ladder **63 s**, `test-census.R` **259 s**, `test-mutant.R` **648 s**, `test-gradient-incidence.R` **542 s**, `test-gradient-parity.R` **329 s** — about **31 minutes** a leg, down from roughly 55. The cut came from asking what each fixture's patch lifetime buys: three census checks needed a distribution with live density at the bottom and each GREW one at 124–251 s, where seeding three cohorts into the initial state reaches 78 live nodes in 0.77 s with more margin on every assertion; and the TF24 mutant identity justified a lifetime of 14 by a bug whose fix (odelia 0.4.0's step subdivision) this branch deliberately removed, so it now runs at 6 — 12,714 replayed steps agreeing to 8e-13 against a 1e-3 tolerance, with the step count asserted rather than implied. ⚠️ **What is left is not slack.** `test-census.R`'s G3 keeps a grown lifetime-12 stand because it measurably fails below it (2.09e-04 against a 1e-4 tolerance at 5), and incidence and parity are the only route to "how often". Whether a maintainer wants 31 minutes a leg or a gated tier is theirs. ⚠️ **`test-review.md` root-causes this** — two mechanisms, a stiffness cliff just past a patch lifetime of 3 and a sweep linear in recorded steps — and records a numerical disagreement found while measuring it. |
