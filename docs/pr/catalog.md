...
`src/gradient_ladder.cpp` is **30 entry points**, down from 33. The two block name
...
formed at, and `ladder_node_count_tf24` is gone because `Species$size` answers it
from R. The prefix **partitions** them now, which the file's own header used
