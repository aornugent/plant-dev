...
| **run_mutant** | **Restore it, or ship regressed against `develop`.** | `SCM::run_mutant` was a `stop()` here and works on develop, where #643 restored it for TF24. ⚠️ **The comment naming odelia's `ReplaysField` as the replacement had it backwards.** `ReplaysField` is what was *replaced*: odelia's `9e84ef6` split `Replayable` into `RecordsSteps` + `ReplaysField`, and `001528b` deleted both four days later for `recorded_stage{step,stage}` + `RecordsChoices` (today's `SolvesForValues`). Plant's comment was written on `a02588c1`, twenty days after that deletion. The concept survives only on odelia's `ad/merge-upstream` and `backup/pre-restructure-2026-09-09`; it is in no tag. odelia issue **#47** is the live ticket for field replay and **scopes it out**. So restoring develop's mechanism means putting three hooks back into odelia (`ode::cache(System&)`, `ode::cache(System&,int)`, `ode::load(System&)`, gated by `has_cache`) against a deliberate deletion, plus `Control$save_RK45_cache` and its bindings. <br><br>**In progress instead:** the patch keeps the field itself, in `Patch::set_ode_state` — the one place it is built — so nothing depends on the solver offering a hook. Single-species identity replays bit for bit, as does 2 residents → 2 invaders; **1 resident → 2 invaders does not**, and invader independence (N together = N separately, since a fixed field cannot be moved) is the acceptance test it has to pass. Until it does, `test-mutant.R` stays skipped and plant's Known issues says so. |
