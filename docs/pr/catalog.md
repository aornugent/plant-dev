...
| `plant` `test-gradient-incidence.R` / `test-gradient-parity.R` **[v, fixed]**, and a NEW finding under it | **The fixtures are fixed and the reference found a defect.** Every driver answers on the bounded pool, so the three lists naming stands whose descent overflows are empty, `seasonal` answers and `shaded` reaches `shade-death` again; incidence **66 pass 0 fail**, parity **52 pass 0 fail**. The dry-pin band moved with the path integral's SHAPE rather than its level -- raising resistance uniformly takes the share to zero, while sweeping `D_c` at a held 1 m anchor takes it 0.90 -> 54.65 per cent as the shared soil column drops 0.13204 -> 0.13017. ⚠️ **Refereeing `shaded` and `clamped` for the first time found a wrong gradient row**: ten columns, every one `theta` or `omega`, off by ~1 per cent, with the census provably smooth there (fitted slope 1105.19 against the sweep's 1094.71) and a forward tangent reproducing the sweep to 1e-11. Not localised, and the rung holds an exact expected count rather than a widened floor. See `docs/pr/transcription.md`. |
...
| **[v]** | **`test-scm.R` is fixed; `test-canopy-methods.R` is a later patch's.** Measured per file against the **552 s** everything this branch adds costs: `test-scm.R` was **1410 s** and `test-canopy-methods.R` is **1620 s**. `test-scm.R` was one block — *"A second run on one SCM reproduces the first"*, **1402 of its 1410 s** — running TF24 twice at `max_patch_lifetime = 10`, three times past the stiffness cliff. **The block is now lifetime 2 and the file is 7.6 s with no failures**, its introduction schedule deliberately unthinned because those 81 cohorts are what its final state comparison compares. What it guards is `#585`'s defect: `reset()` must reach `Environment::clear()`, whose `clear_state()` puts the soil and the flux accumulators back where the run began. That is a property of the reset, not of the trajectory. ⚠️ **The shortening was checked by mutation, not by a green run.** Against a build with `clear_state()` reverted the block fails on the exact contamination — after the reset the soil reads `0.305` in all five layers with flux accumulators `2.000 1.851 1.633 0.081`, against a starting `0.214` and zeros. A second mutant removing only the state restore and keeping the cache invalidation fails the same way, so it is the restore the block is holding. The block also gained the assertion it never had: that the first run moved the environment at all, without which a fixture too short to move anything passes both reset checks while guarding nothing. ⚠️ **`store_trajectory records one state per instruction` is a second guard on the same restore** and fails on both mutants, because `store_trajectory()` runs the SCM again when the run kept no states. **Whether this branch made TF24 slower than `develop`: not a regression at the one lifetime measured.** `develop`'s three packages built in a private library (odelia 0.3.1, phylloptim 0.8.0), same fixture, same sitting, at lifetime 3: 203 steps/2.46 s against this branch's 205/3.00 on the default density coordinate, and 213/1.35 against 210/1.48 on the birth-date one. Single readings, so read the step counts and not the seconds. `test-canopy-methods.R` is not split; its eighteen inherited blocks are FF16-only at lifetimes 40 and ~105, and FF16 is not templated here — but its three TF24 blocks are the same recipe this one was, at lifetime 8. |
