...
| **[v]** | **What the new tests add to every `R CMD check` leg, on three operating systems.** `tests/testthat.R` is `test_check("plant")`, so all of this runs in the check — the bespoke ladder runner is a developer convenience, not a gate. Measured today: the ladder **63 s**, `test-census.R` **~12.5 min** after the fixture cut below. Recorded earlier in this document: `test-gradient-incidence.R` **542 s**, `test-gradient-parity.R` **329 s**, and `test-mutant.R`'s TF24 block a **945 s** resident run plus a **597 s** replay. That totals roughly **50 minutes** per leg. ⚠️ **None of it is waste** — every fixture above carries a comment saying what its lifetime buys, and the three that looked slack were cut: `solved_stand()`'s default went 20 → 5 (measured: 98 nodes in 274 s against 88 in 124 s, so ten nodes for 2.2× the time) and two structural blocks took a short schedule at 0.8 s each, halving that file. What is left is the price of refereeing a transpose at a stand's scale, and whether a maintainer wants to pay it on every leg or gate part of it is theirs. |
