...
| **`gradient_ladder.cpp`** | ~~Ship 24 test-only entry points~~ — **STUDIED: the decision is four, not twenty-four.** | **28 `[[Rcpp::export]]`, 22 of them `ladder_`.** The test applied to each: can the quantity be had from the published R interface? `stand_gradient` returns one number per (metric, trait) over a whole run, so a Jacobian, an adjoint, or a reference evaluated at one state has no route through it — that is what earns a place. ⚠️ **Two failed the test and are deleted.** `ladder_field_knots_tf24` is `patch$environment$light_availability$state`, bit-identical in height and value on all six patch fixtures, and its one consumer never read the slope column where the two differ by 1.4e-17. `ladder_range_base_state_tf24` is the first record `store_trajectory()` keeps, or the introduction record that widened into it — bit-identical over 45 ranges on four stands, against an export whose comment claimed *"a caller cannot read it off the trajectory"*. The ladder is unchanged at 615 pass, 0 fail, 0 skip. ⚠️ **Eighteen are strictly necessary** and the reasoning is the same for each: they are the forward and reverse maps at one point (the five block calls, the five right-hand-side calls, the introduction Jacobian), the references at a different scalar (the trajectory tangent, the initial-state tangent and its replay, the trait difference), or the boundary node's own row and count — which nothing downstream reports, and which `test-census.R` now shows is a live channel in the seed. Measured, not assumed: `ladder_rhs_value_forward_tf24` is bit-identical to the published `patch$ode_rates` on four fixtures, which is that check's RESULT rather than grounds to delete it, since the comparison needs both sides. ⚠️ **What is left to decide is four.** Three have a user's question behind them and belong under the `census_` prefix if a maintainer agrees the package answers it: `ladder_census_trait_direct_tf24` is the allometric term, the other half of the one recording whose state half ships as `stand_census_state_adjoint`; `ladder_clamp_counts_differentiated_tf24` is the only route to whether a gradient carries a severance, where the published forward tally says only that the guard is reachable; `ladder_curvature_margin_tf24` is the distance to a floor that decides which rows exist. The fourth, `ladder_trait_names_tf24`, is a one-line duplicate of `census_trait_names_tf24` that differs only in taking a Patch, and goes if `trait_adjoint_names` is exposed on `Patch` in `RcppR6_classes.yml`. ⚠️ **The macro-guard reason is unchanged**: `Rcpp::compileAttributes()` rewrites `src/RcppExports.cpp` whole and unconditionally, so a guard here without a matching one there is an undefined symbol at link time and a guard there is lost on the next regeneration. So is the install cost — a quarter of the build is in this one file.
...
| **[v]** | **What the new tests add to every `R CMD check` leg, on three operating systems.** `tests/testthat.R` is `test_check("plant")`, so all of it runs in the check — the ladder runner is a developer convenience, not a gate. Measured end to end, per file: ladder **61 s**, `test-census.R` **1.4 s**, `test-mutant.R` **80 s**, `test-gradient-incidence.R` **253 s**, `test-gradient-parity.R` **157 s** — about **9 minutes** a leg, down from 31 and from roughly 55 before that. The cut came from one mechanism and three structural findings, all in `test-review.md`. ⚠️ **The mechanism is that a patch lifetime buys the stiff regime and the COHORT COUNT is what it costs**, and the default schedule confounds them by deriving its introduction count from the lifetime: thinned to twenty introductions, the TF24 mutant replay is 6071 recorded steps in 71 s against 12714 in 623, incidence's clamped stand is 18 s against 258, and parity's `shaded` is 22 s against 319 — each keeping every property its block asserts. ⚠️ **Three checks could not fail and now can**: a block cleared the diagnostics on a stand three others read, so the light floor's "does not bind at the shipped value" was asserted against a zeroed tally; a cache keyed on whether a stand had been swept ran two of five drivers twice; and `incidence_run` and `parity_stand` were two copies of one recipe, against parity's own comment that the regimes are named once. ⚠️ **What is left is A1's or D's** — incidence's dry-pin block is 178 s and its `expect_lt(share, 5)` is one of A1's six failures, and parity's `seasonal` keeps its whole schedule because it is that file's only route to a hydraulic shutdown.
| **[v]** | **Two files outside this diff's new tests cost fifty minutes a leg, and a later patch's.** Measured per file against the **552 s** everything this branch adds now costs: `test-scm.R` **1410 s** and `test-canopy-methods.R` **1620 s**. ⚠️ **`test-scm.R` is one block, and the block is byte-identical to `develop`** — *"A second run on one SCM reproduces the first"* is **1402 of its 1410 s**, running TF24 twice at `max_patch_lifetime = 10` on `trait_matrix(0.1978791, "lma")`: 93 introductions, 22,597 recorded steps, three times past the stiffness cliff and on a bare single-trait recipe, which `test-review.md` measures as 14x stiffer in steps than the same traits through `TF24_hyperpar`. Its own comment reads *"Short patch lifetime: this covers the reset, not the trajectory"*, and all three of its assertions — that a reset clears the environment state, and that a second run reproduces the first's offspring and state — hold identically at a lifetime of **2 in 4.8 s** and of **1 in 2.9 s**. ⚠️ **Whether this branch made it slower is NOT measured**: the block is upstream's but the model under it is this branch's TF24, and answering it needs `develop`'s three packages built in a private library. `test-canopy-methods.R` is not split; its eighteen inherited blocks are FF16-only at lifetimes 40 and ~105, and FF16 is not templated here. |
...
**partitions** them, which the file's own header used to flag as wrong: 22 are
`ladder_`, called from `tests/testthat/` and nowhere else, and the 6 spelled
`census_` are exactly the ones `NEWS.md` publishes with migration lines.
