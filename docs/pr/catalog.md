...
`test-gradient-demo.R` in the diff; the net **+237 C++ and +62 R names** against a
brief of net deletion; plant's own headers compiled with `-isystem` so ~95% of the
package is un-warned; and `gradient_control()`'s check that cannot fail, which is
CONFIRMED LIVE and costed in its row. ⚠️ **Two are struck**: the `[r]` pair of dead
assertions are both stale -- the files were restructured and one of them records
that very defect and its repair -- and `[r]` phylloptim not compiling against odelia
master is landing order rather than a decision.
...
| **[v→D from E]** | **Does the diff carry `test-gradient-demo.R` (116 lines)?** ⚠️ **It does not SKIP under `R CMD check` -- it is not in the tarball at all.** `.Rbuildignore:37` names the file itself, not just the `overstorey_staging/` helpers at `:17-18`, so the built package's `tests/testthat/` has no such file and the check's own skip list names only `test-pm-leaf-demo.R`. In the developer loop (`run-tests.sh` sets `NOT_CRAN=true`) it runs: **32 pass, 0 skip**. So the question is narrower than it was: a maintainer building the tarball never sees this file, and only a reader of the git diff does. That is `gradient_ladder.cpp`'s question again -- what the pull request CONTAINS, not what it runs. |
...
| **[v, CONFIRMED LIVE]** | **`gradient_control()` has a check that cannot fail.** It returns five values positionally and R names them; `ci_abs_tol` and `gradient_curvature_floor` are both `1e-3` at defaults, so transposing them passes both assertions while `stand_gradient_compare()` refuses on the wrong pair. ⚠️ **The values and the names are in different files** -- `scm.h:284` builds the five in order, `R/stand_gradient.R:49` names them in order -- and `census_gradient.cpp:85`, three lines above the export, states the rule this breaks: *"a name is what crosses: a position means a different metric as soon as the model's list changes."* The fix is to return them paired from `scm.h`, which the export then hands over named and the R wrapper stops renaming. **Costed rather than done**: it changes an export's return type, so it wants `compileAttributes()`, a full plant rebuild and a ladder re-run. |
