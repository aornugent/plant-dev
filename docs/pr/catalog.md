...
package is un-warned; `gradient_control()`'s check that cannot fail; `[r]` two
more assertions that cannot fail; and `[r]` phylloptim not compiling against
odelia master, which is landing order rather than a decision.
