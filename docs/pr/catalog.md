...
| ~~`plant` `test-gradient-incidence.R` / `test-gradient-parity.R`~~ **[v, fixed]** | **Most of it was the pool, and the rest was measured.** With the pool bounded every driver answers, so the three lists naming the stands whose descent overflows are empty; `seasonal` answers and `shaded` reaches `shade-death` again. The dry-pin share is the one part that was not the pool: the stem path integral moved it, measured at **0.90% with `D_c`, `theta_c` and `L_tip` at zero and `K_s` at 1 against 54.65% as shipped**, on the same stand. Incidence **66 pass 0 fail**, parity **52 pass 0 fail**, and the ladder's captured reference now referees all five regimes for the first time. |
