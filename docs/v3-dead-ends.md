...
| **"The `Leaf` blocker requires owning the leaf per unit"** | the drift was real, its cause was confirmed, and the two options on record were "copy the Strategy per unit" or "audit every leaf cache" — with the copy preferred as structural | **Refuted by measuring the other path.** The blocker was only ever measured on the whole-`Patch` **copy** path; the design's unit calls `r_set_state`, which begins with `reset()`. On that path deferred and inline replays are **identical** (1.11e-13 / 7.64e-12 / 8.69e-11 at segments 20/40/60) while the copy path still splits, so the probe's detecting power is intact and there is simply nothing to detect. Neither option was needed. `v3-facts.md` §4f |
| **"…and `reset()` works by wiping the leaf to `NA`"** | my own predicted mechanism, from reading `prepare_strategy` → `leaf = Leaf(...)` → `setup_clean_leaf()` | **Wrong, though the conclusion held.** Measured, `ci_` goes 26.479476 → **27.27942 / 27.19419 / 27.11116** at segments 20/40/60 — not `NA`, and *segment-specific*. The leaf is reconstructed **and then re-solved against the restored state**, so it is a function of the unit rather than of history. Second time in one session that a correct conclusion rested on a wrong stated mechanism (the other was `MeanLight`); **the object diff is what settled both** |
| **"All species must share `eta`" (mine, stated as a hard constraint)** | measured that mixing η makes the field diverge — 7.98e+14 against an exact 0.118 — and `patch.h:757-759` takes the query factors from `species[0]` alone | **Refuted as a framing, and it was the "leaking solutions into requirements" error running in reverse: an implementation limitation written up as a domain constraint.** `X(eta)` is in the AD field macros of **all three** models (`ff16_strategy.h:113`, `k93_strategy.h:49`, `tf24_strategy.h:98`), so plant already declares η a differentiation target, and community assembly plus selection gradients (user stories 6.1/6.2) vary traits *across* species by definition. **Separability survives per-species η**: grouping sources by η gives `A(z) = Σ_η Σ_p a_p^η(z)·[Σ_{j:η_j=η} ampM_j b_p^η(H_j)]`, a rank **3·n_η** field that degenerates to today's rank 3 when η is shared. So the measurement stands and the conclusion inverts — it is a **defect to fix** (`v3-requirements.md` C6.4/C6.4b, U15), and the forward value is wrong too, not only the gradient |
